defmodule Sorter do
  ...
  def dft([], res, nil, key), do: nil
  def dft(stack, res, nil, key) do
    {curr, remain} = List.pop_at(stack, -1)
    found = find(curr.location, key)
    res = [curr.location | res]
    [remain | Enum.reverse(curr.children)] 
    |> List.flatten()
    |> dft(res, found, key)
  end
  def dft(stack, res, found, key), do: found
