from kleast_numbers import get_kleast_recursion
...
def test_normal_recursion():
    lst = [4, 5, 1, 3, 2]
    assert get_kleast_recursion(lst, 0) == []
    assert get_kleast_recursion(lst, 1) == [1]
    assert get_kleast_recursion(lst, 2) == [1,2]
    assert get_kleast_recursion(lst, 3) == [1,3,2]
    assert get_kleast_recursion(lst, 5) == [4,1,3,2,5]
    assert get_kleast_recursion(lst, 6) == []
...
def test_part_repeat_recursion():
    ...
    assert get_kleast_recursion(lst, 1) == [1]
    assert get_kleast_recursion(lst, 2) == [1,2]
    assert get_kleast_recursion(lst, 3) == [1,2,2]
    assert get_kleast_recursion(lst, 5) == [4,1,2,2,5]
    assert get_kleast_recursion(lst, 9) == []

def test_multi_repeat_recursion():
    lst = [2,2,1,1,3,3]
    assert get_kleast_recursion(lst, 0) == []
    assert get_kleast_recursion(lst, 1) == [1]
    assert get_kleast_recursion(lst, 2) == [1,1]
    assert get_kleast_recursion(lst, 3) == [1,1,2]
    assert get_kleast_recursion(lst, 5) == [2,2,1,1,3]
    assert get_kleast_recursion(lst, 7) == []

def test_all_repeat_recursion():
    lst = [2,2,2,2,2,2]
    assert get_kleast_recursion(lst, 0) == []
    assert get_kleast_recursion(lst, 1) == [2]
    assert get_kleast_recursion(lst, 2) == [2,2]
    assert get_kleast_recursion(lst, 3) == [2,2,2]
    assert get_kleast_recursion(lst, 7) == []
...
def test_one_recursion():
    ...
    assert get_kleast_recursion(lst, 0) == []
    assert get_kleast_recursion(lst, 1) == [1]
    assert get_kleast_recursion(lst, 2) == []

def test_none_recursion():
    lst = []
    assert get_kleast_recursion(lst, 0) == []
    assert get_kleast_recursion(lst, 1) == []
